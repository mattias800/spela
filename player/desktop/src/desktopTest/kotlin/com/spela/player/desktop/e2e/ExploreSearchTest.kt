...
class ExploreSearchTest {
    ...
    @Test
    fun searchBarRendersAndNavigatesToGlobalSearchScreen() = runComposeUiTest {
    ...
    @Test
    fun filterPanelShowsSectionsAndUpdatesChipSelections() = runComposeUiTest {
        val harness = createHarness()
        ...
        val snesFilters = harness.exploreViewModel.gameSearchState.value.filters
        assertEquals(listOf("SNES"), snesFilters.consoles)

        // Click NES chip too
        onNodeWithTag("filter_console_chip_NES").performClick()
        advanceQuick(harness)

        val snesAndNesFilters = harness.exploreViewModel.gameSearchState.value.filters
        assertEquals(listOf("SNES", "NES"), snesAndNesFilters.consoles)

        // Toggle SNES off
        onNodeWithTag("filter_console_chip_SNES").performClick()
        advanceQuick(harness)

        val nesOnlyFilters = harness.exploreViewModel.gameSearchState.value.filters
        assertEquals(listOf("NES"), nesOnlyFilters.consoles)

        // Click RPG chip
        onNodeWithTag("filter_genre_chip_RPG").performClick()
        advanceQuick(harness)

        val genreFilters = harness.exploreViewModel.gameSearchState.value.filters
        assertEquals(listOf("RPG"), genreFilters.genres)

        // Select "unplayed"
        onNodeWithTag("filter_play_status_chip_unplayed").performClick()
        advanceQuick(harness)

        val unplayedFilters = harness.exploreViewModel.gameSearchState.value.filters
        assertEquals("unplayed", unplayedFilters.playStatus)

        // Clicking same chip deselects
        onNodeWithTag("filter_play_status_chip_unplayed").performClick()
        advanceQuick(harness)

        val deselectedPlayStatusFilters = harness.exploreViewModel.gameSearchState.value.filters
        assertEquals("", deselectedPlayStatusFilters.playStatus)
    ...
    @Test
    fun applyFiltersShowsResultsAndClearFiltersResetsState() = runComposeUiTest {
        val harness = createHarness()
    ...
    @Test
    fun savedSearchesRenderAndDelete() = runComposeUiTest {
        val harness = createHarness()
