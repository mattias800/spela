#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"

# ── Parse arguments ──

FILTER=""
RERUN=false

while [ $# -gt 0 ]; do
  case "$1" in
    --rerun) RERUN=true; shift ;;
    *) FILTER="$1"; shift ;;
  esac
done

# ── Build gradle args ──

GRADLE_ARGS=":android:testDebugUnitTest"

if [ -n "$FILTER" ]; then
  GRADLE_ARGS="$GRADLE_ARGS --tests $FILTER"
  echo "Running Android unit tests matching: $FILTER"
else
  echo "Running all Android unit tests..."
fi

if $RERUN; then
  GRADLE_ARGS="$GRADLE_ARGS --rerun-tasks"
fi

# ── Run tests ──

"$SCRIPT_DIR/gradlew" $GRADLE_ARGS --console=plain 2>&1

# ── Show summary ──

echo ""
echo "── Test Results ──"
RESULT_DIR="$SCRIPT_DIR/android/build/test-results/testDebugUnitTest"
if [ -d "$RESULT_DIR" ]; then
  TOTAL=0
  FAILURES=0
  ERRORS=0
  for f in "$RESULT_DIR"/TEST-*.xml; do
    [ -f "$f" ] || continue
    t=$(xmllint --xpath "string(/testsuite/@tests)" "$f" 2>/dev/null || echo 0)
    fl=$(xmllint --xpath "string(/testsuite/@failures)" "$f" 2>/dev/null || echo 0)
    e=$(xmllint --xpath "string(/testsuite/@errors)" "$f" 2>/dev/null || echo 0)
    time=$(xmllint --xpath "string(/testsuite/@time)" "$f" 2>/dev/null || echo "?")
    name=$(basename "$f" .xml | sed 's/TEST-//')
    TOTAL=$((TOTAL + t))
    FAILURES=$((FAILURES + fl))
    ERRORS=$((ERRORS + e))
    if [ "$fl" -gt 0 ] || [ "$e" -gt 0 ]; then
      echo "  FAIL  $name ($t tests, ${fl} failures, ${e} errors) ${time}s"
    else
      echo "  PASS  $name ($t tests) ${time}s"
    fi
  done
  echo ""
  echo "Total: $TOTAL tests, $FAILURES failures, $ERRORS errors"
fi
