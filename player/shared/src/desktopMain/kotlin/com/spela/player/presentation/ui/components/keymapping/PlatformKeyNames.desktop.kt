package com.spela.player.presentation.ui.components.keymapping

import androidx.compose.ui.input.key.Key

/**
 * Desktop implementation: maps Compose Key.keyCode values to human-readable names.
 * Key codes stored in the mapping are the Long keyCode cast to Int.
 */
actual fun platformKeyName(keyCode: Int): String {
    val key = Key(keyCode.toLong())
    return desktopKeyNames[key] ?: "Key $keyCode"
}

private val desktopKeyNames = mapOf(
    // Letters
    Key.A to "A", Key.B to "B", Key.C to "C", Key.D to "D",
    Key.E to "E", Key.F to "F", Key.G to "G", Key.H to "H",
    Key.I to "I", Key.J to "J", Key.K to "K", Key.L to "L",
    Key.M to "M", Key.N to "N", Key.O to "O", Key.P to "P",
    Key.Q to "Q", Key.R to "R", Key.S to "S", Key.T to "T",
    Key.U to "U", Key.V to "V", Key.W to "W", Key.X to "X",
    Key.Y to "Y", Key.Z to "Z",

    // Numbers
    Key.Zero to "0", Key.One to "1", Key.Two to "2", Key.Three to "3",
    Key.Four to "4", Key.Five to "5", Key.Six to "6", Key.Seven to "7",
    Key.Eight to "8", Key.Nine to "9",

    // Arrow keys
    Key.DirectionUp to "Up",
    Key.DirectionDown to "Down",
    Key.DirectionLeft to "Left",
    Key.DirectionRight to "Right",

    // Modifiers and special keys
    Key.Enter to "Enter",
    Key.Spacebar to "Space",
    Key.Escape to "Esc",
    Key.Tab to "Tab",
    Key.Backspace to "Backspace",
    Key.Delete to "Delete",
    Key.ShiftLeft to "L-Shift",
    Key.ShiftRight to "R-Shift",
    Key.CtrlLeft to "L-Ctrl",
    Key.CtrlRight to "R-Ctrl",
    Key.AltLeft to "L-Alt",
    Key.AltRight to "R-Alt",

    // Function keys
    Key.F1 to "F1", Key.F2 to "F2", Key.F3 to "F3", Key.F4 to "F4",
    Key.F5 to "F5", Key.F6 to "F6", Key.F7 to "F7", Key.F8 to "F8",
    Key.F9 to "F9", Key.F10 to "F10", Key.F11 to "F11", Key.F12 to "F12",

    // Punctuation
    Key.Comma to ",",
    Key.Period to ".",
    Key.Semicolon to ";",
    Key.Apostrophe to "'",
    Key.LeftBracket to "[",
    Key.RightBracket to "]",
    Key.Slash to "/",
    Key.Backslash to "\\",
    Key.Minus to "-",
    Key.Equals to "=",
    Key.Grave to "`",

    // Home/End/Page
    Key.Home to "Home",
    Key.MoveEnd to "End",
    Key.PageUp to "PgUp",
    Key.PageDown to "PgDn",
    Key.Insert to "Insert",
)
