...
func TestNonPlayable_ListSaves_Blocked(t *testing.T) {
	env := setupNonPlayableTestEnv(t)

	w := httptest.NewRecorder()
	req := httptest.NewRequest("GET", "/api/games/"+env.nonPlayGameID+"/saves", nil)
...
func TestNonPlayable_ListSaves_Allowed(t *testing.T) {
	env := setupNonPlayableTestEnv(t)

	w := httptest.NewRecorder()
	req := httptest.NewRequest("GET", "/api/games/"+env.playableGameID+"/saves", nil)
	req.Header.Set("Authorization", "Bearer "+env.token)
	env.router.ServeHTTP(w, req)
...
func TestNonPlayable_UploadSave_Blocked(t *testing.T) {
	env := setupNonPlayableTestEnv(t)
	...
	w := httptest.NewRecorder()
	req := httptest.NewRequest("POST", "/api/games/"+env.nonPlayGameID+"/saves", &buf)
	req.Header.Set("Authorization", "Bearer "+env.token)
...
func TestNonPlayable_UploadAutoSave_Blocked(t *testing.T) {
	env := setupNonPlayableTestEnv(t)

	var buf bytes.Buffer
	writer := multipart.NewWriter(&buf)
	part, _ := writer.CreateFormFile("save", "autosave.sav")
	part.Write([]byte("fake auto save"))
	writer.Close()

	w := httptest.NewRecorder()
	req := httptest.NewRequest("POST", "/api/games/"+env.nonPlayGameID+"/saves/auto", &buf)
	req.Header.Set("Authorization", "Bearer "+env.token)
	req.Header.Set("Content-Type", writer.FormDataContentType())
	env.router.ServeHTTP(w, req)
...
func TestNonPlayable_GetAutoSave_Blocked(t *testing.T) {
	env := setupNonPlayableTestEnv(t)

	w := httptest.NewRecorder()
	req := httptest.NewRequest("GET", "/api/games/"+env.nonPlayGameID+"/saves/auto", nil)
	req.Header.Set("Authorization", "Bearer "+env.token)
	env.router.ServeHTTP(w, req)
...
// --- Save data (SRAM) endpoints ---

func TestNonPlayable_ListSaveData_Blocked(t *testing.T) {
	env := setupNonPlayableTestEnv(t)

	w := httptest.NewRecorder()
	req := httptest.NewRequest("GET", "/api/games/"+env.nonPlayGameID+"/save-data", nil)
	req.Header.Set("Authorization", "Bearer "+env.token)
	env.router.ServeHTTP(w, req)
...
func TestNonPlayable_UploadSaveData_Blocked(t *testing.T) {
	env := setupNonPlayableTestEnv(t)

	var buf bytes.Buffer
	writer := multipart.NewWriter(&buf)
	writer.WriteField("name", "SRAM Save")
	part, _ := writer.CreateFormFile("file", "save.srm")
	part.Write([]byte("sram data"))
	writer.Close()

	w := httptest.NewRecorder()
	req := httptest.NewRequest("POST", "/api/games/"+env.nonPlayGameID+"/save-data", &buf)
	req.Header.Set("Authorization", "Bearer "+env.token)
	req.Header.Set("Content-Type", writer.FormDataContentType())
	env.router.ServeHTTP(w, req)
...
func TestNonPlayable_UploadActiveSaveData_Blocked(t *testing.T) {
	env := setupNonPlayableTestEnv(t)

	var buf bytes.Buffer
	writer := multipart.NewWriter(&buf)
	part, _ := writer.CreateFormFile("file", "active_sram.sav")
	part.Write([]byte("active sram data"))
	writer.Close()

	w := httptest.NewRecorder()
	req := httptest.NewRequest("POST", "/api/games/"+env.nonPlayGameID+"/save-data/active", &buf)
	req.Header.Set("Authorization", "Bearer "+env.token)
	req.Header.Set("Content-Type", writer.FormDataContentType())
	env.router.ServeHTTP(w, req)
...
func TestNonPlayable_DownloadActiveSaveData_Blocked(t *testing.T) {
	env := setupNonPlayableTestEnv(t)

	w := httptest.NewRecorder()
	req := httptest.NewRequest("GET", "/api/games/"+env.nonPlayGameID+"/save-data/active", nil)
	req.Header.Set("Authorization", "Bearer "+env.token)
	env.router.ServeHTTP(w, req)
