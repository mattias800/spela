...
func (h *ExploreHandler) HumaGetScreenshotGallery(_ context.Context, in *GetScreenshotGalleryInput) (*GetScreenshotGalleryOutput, error) {
	...
	if err := baseQuery.
		Select("game_screenshots.url, games.id as game_id, games.title as game_title, LOWER(consoles.abbreviation) as console_abbr").
	...
	for _, r := range rows {
		items = append(items, ScreenshotItem{
			...
			ConsoleColor: db.ConsoleColorTheme(r.ConsoleAbbr), // registry-derived (#1443)
		})
	}
...
func (h *ExploreHandler) HumaGetArtworkGallery(_ context.Context, in *GetArtworkGalleryInput) (*GetArtworkGalleryOutput, error) {
	...
	var rows []artworkRow
	if err := h.DB.Table("game_artwork_images").
		Select("game_artwork_images.igdb_image_id, game_artwork_images.local_path, game_artwork_images.width, game_artwork_images.height, games.id as game_id, games.title as game_title, games.rating, LOWER(consoles.abbreviation) as console_abbr").
		Joins("JOIN games ON games.id = game_artwork_images.game_id AND games.deleted_at IS NULL AND games.is_primary = true").
		Joins("JOIN consoles ON consoles.id = games.console_id AND consoles.deleted_at IS NULL").
...
func (h *ExploreHandler) HumaGetCoverGallery(_ context.Context, in *GetCoverGalleryInput) (*GetCoverGalleryOutput, error) {
	...
	var rows []coverRow
	if err := baseQuery.
		Select("games.cover_url, games.id as game_id, games.title as game_title, games.rating, LOWER(consoles.abbreviation) as console_abbr").
		Order("games.rating DESC").
		Offset(offset).
		Limit(limit).
		Scan(&rows).Error; err != nil {
	...
	for _, r := range rows {
		// console name / colour / cover-aspect are registry-derived (#1443).
		items = append(items, CoverItem{
			CoverURL:          resolveImageURL(r.CoverURL),
			GameID:            strconv.FormatUint(uint64(r.GameID), 10),
			GameTitle:         r.GameTitle,
			ConsoleName:       db.ConsoleName(r.ConsoleAbbr),
			ConsoleAbbr:       r.ConsoleAbbr,
			ConsoleColor:      db.ConsoleColorTheme(r.ConsoleAbbr),
