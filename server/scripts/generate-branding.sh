#!/usr/bin/env bash
# Generates optimized branding assets from the source logo.
# The source image (branding/spela-logo-original.png) is the source of truth.
...
SOURCE="$REPO_ROOT/branding/spela-logo-original.png"
...
# Crop to content (trim transparent border), then generate sized variants.
BBOX=$(magick "$SOURCE" -alpha extract -threshold 1% -negate -format '%@' info:)
echo "Content bounding box: $BBOX"
...
magick "$SOURCE" -crop "$BBOX" +repage -resize 512x -strip PNG32:"$DEST_BACKEND"
...
magick "$SOURCE" -crop "$BBOX" +repage -resize 512x -strip PNG32:"$DEST_PLAYER"
echo "  Player logo:  $DEST_PLAYER ($(du -h "$DEST_PLAYER" | cut -f1))"

# README logo (wider, for GitHub display)
DEST_README="$REPO_ROOT/docs/spela-logo.png"
mkdir -p "$(dirname "$DEST_README")"
magick "$SOURCE" -crop "$BBOX" +repage -resize 400x -strip PNG32:"$DEST_README"
