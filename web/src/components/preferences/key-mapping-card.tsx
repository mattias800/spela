...
interface KeyMappingCardProps {
  ...
  onConsoleKeyMappingChange: (consoleId: string, mapping: string, customMapping?: Record<string, string>) => void;
}
...
export function KeyMappingCard({
  ...
  const selectedMode = (preferences?.selectedKeyMapping ?? "arrows-left") as KeyMappingMode;
  const customMapping = preferences?.customKeyMapping ?? {};

  const activeMapping = selectedMode === "custom"
      ...
      <CardContent>
        {isLoading ? (
          ...
          <div className="space-y-6">
            {/* Mode selector */}
            <div>
              <p className="text-sm font-medium text-surface-300 mb-3">Global Default</p>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-surface-800">
                        ...
                        <th className="text-left px-3 py-2 text-xs font-semibold uppercase tracking-wider text-surface-400">Key Mapping</th>
                      </tr>
                    </thead>
                    <tbody>
                      {consoles.map((console) => {
                        const consoleKM: ConsoleKeyMapping | undefined = preferences?.consoleKeyMappings[console.id];
                        return (
                          <tr key={console.id} className="border-b border-surface-800/50">
                            ...
                            <td className="px-3 py-2">
                              <select
                                ...
                                onChange={(e) => onConsoleKeyMappingChange(console.id, e.target.value)}
                              >
