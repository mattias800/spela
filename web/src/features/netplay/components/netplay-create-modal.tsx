...
export function NetplayCreateModal({
  ...
  const supportedGames = games.filter((g) =>
    NETPLAY_SUPPORTED_CONSOLES.includes(g.consoleName),
  );
                ...
                <div className="absolute z-10 top-full left-0 right-0 mt-1 rounded-lg bg-surface-900 border border-surface-700 shadow-xl max-h-64 overflow-y-auto">
                  {supportedGames.length === 0 ? (
                    <p className="px-3 py-3 text-sm text-surface-500">
                      {games.length > 0
                        ? "No supported netplay games found. Supported consoles: NES, SNES, GB, GBC, GBA, Genesis, Mega Drive."
                        : "No games found"}
                    </p>
                  ) : (
